...
const NOTE_TILE_CLASS =
  "aspect-[1/1.06] min-h-[180px] rounded-md border p-3.5";
      ...
      <div className="grid w-full gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
        {notes.map((note, index) => {
          ...
          return (
            <article
              key={note.id}
              className={`${NOTE_TILE_CLASS} flex flex-col gap-3 overflow-hidden text-[#2f2a1f] ${style.paper} ${style.border}`}
            >
              {isEditing ? (
                <>
        ...
        {isComposerOpen ? (
          <form
            onSubmit={createNote}
            className={`${NOTE_TILE_CLASS} flex flex-col gap-3 border-[#e6cb64] bg-[#fff2ad] text-[#2f2a1f]`}
          >
