...
function EventChip({
  ...
  const className = `flex h-5 w-full items-center gap-1.5 rounded-[4px] px-1.5 ${style.chip}`;
...
function DayCell({
  ...
  return (
    <div
      className={`relative min-h-0 min-w-0 overflow-hidden border-b border-r border-[#eee9df] p-1.5 ${
        cell.outside ? "bg-[#fbfaf7] text-[#ada79e]" : "bg-[#fffdf9]"
      } ${isToday ? "z-[1] bg-[#f4f6ff] ring-2 ring-inset ring-[#6682fa]" : ""}`}
    >
